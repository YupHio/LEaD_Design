...
%      \includegraphics[width=0.45\linewidth]{icon for team goes here}
  ...
  {\huge\bfseries LEaD Design: \\ Team Portfolio\par}
	...
	\section{Introduction}
		...
		The University of Idaho has for several years done various projects involving the Tower of Lights Show and equiping the marching band with light-up glasses. The product that is described in this document is for a year-long project for CS 480 and 481: Senior Capstone Design class at University of Idaho. The current "TowerLights" project involves LED-based light bars that are placed in front of front-facing widows of a large buildling (Theophilus Tower Dormitory) and then are illuminated to play animations alongside/syncrhoniously with music. The project is to enhance the current "TowerLights" project. The 'ehnacement' of this project involves the current system's ethernet wiring system in the building to power the LED's, and converting this part of the system to a wireless operation. This in turn requires the reconfigured system to need the development of a wireless module that would be attached to each of the light bars. Thus this module has to sleep and wake up, as well as respond to wireless signals from a computer, and since wireless, these modules will need to be battery powered. Battery power must also be conserved by staying in the sleep state until needed. The purpose of this enhancement is to provide a certain level of portability to have "TowerLights" at other locations. \\
		The product, will give the user the ability to run a program that reads in .tan files and .wav files, have this program communicate with a XBee Wireless module on an Arduino that is attached to a Computer via USB, then communicate wirelessly with each Arduino Receiver, that is batter powered, that is on each LED board, that will then communicate with each LED on that board through wired communication from the Arduino (same one that holds the receiver)to the LEDs. The program that runs through this procedure will be available for the OSX, Windows, and Linux-Based Operating Systems.\\
		This document lives at \url{https://github.com/YupHio/LEaD_Design/tree/master/Doc/TeamPortfolio_LEaD_Design.tex} \\
		The code for the project can be found at \url{https://github.com/YupHio/LEaD_Design/tree/master/Code}
		
		\subsection{Document Purpose}
	 		This document is a team portfolio for the Fall 2017-Spring 2018 CS 480/481: Senior Capstone Design project at the University of Idaho. The purpose of this document is to outline the methodology, design, and record of this project. It defines terms used, outlines the scope of the project, details specific design choices, meeting minutes, project learning, design goals, specification and constraints, system diagrams, analysis of alternatives, engineering modeling, manufacturing/assembly plan, experimental design, data analysis, balance sheet, and other items.
...
\section{Meeting Minutes}
	...
	Current code is all available for use.
	...
	•	Meeting started at 3:30
	•	Fix Compilation issues (lpulse library, as much time as is needed.)
	•	Confirm the towerplayer.cpp and towerarduino.ino, yswavfile.cpp and yswavfile.h are all that is needed for compilation. (15 min)
	•	Bring any remaining hardware questions to Dr. Rinker (10 min)
	•	Clarify the connection, or lack there of, between our senior project and the other tower lights senior design team (as much time as needed)
	•	Do we need the QT GUI code, the other tower lights team received?
	•	Diagramming (UML / Class/ Actor/ ect.) (15 mins)
	•	Meeting ended at 4:42
...
\section{Design Goals}
	...
	The needs of the Client (University of Idaho) are as follows:
		...
		\begin{itemize}
			...
			\item LEDs bright enough to display bright LED
	...
	The goal of the project is to the extend the versatility of the TowerOfLights project, which at the moment, gives the user the ability to run a program which reads in a .tan file (animation files for the lights) and .wav files. Then this
...
\section{System Diagrams}
	...
	The current product flow in regards to the final product is shown below in Figure below. The current setup does not have any battery setup, and requires a wired connection. These enhancements are the core of this project, to enhance the versatility of the TowerOfLights project.
	...
	The desired product flow is shown in the figure below. the main focus is on the battery that should power each Arduino reciever, as well as the SPI protocol from XBee to the Receiver, to make the process wireless instead of wired, which is the main goal of this endeavor.
	...
	The diagram for a flow chart depicting the sequence of actions for running the TowerPlayer program on a Computer is shown in the figure below. This diagram helps with understanding the underlying software that needs to be setup and used before the Hardware can be communicated with
...
\section{Analysis of Alternatives}
	Discussion of possible alternatives and why some alternatives are better is described below. These topics include: safety, moving parts, cost, durability, compatibility, and foolproof.

	\newpage

\section{Engineering Model}
	Discussion of the physical, chemical, and biological system modeling. Also discusses modeling criteria, expected accuracy, and pitfalls. Section of modeling software used is present, as well as data needed and how the data was obtained. and lastly a validation scheme for the model is shown.
