...
Discussion on initial budget, estimated cost for materials, components, labor, and spending plan are all described below.
		...
		{\renewcommand\labelitemi{}
			...
			\begin{itemize}
				...
				\item \textbf{Arduino Sheild}
				Shields are boards that can be plugged on top of the Arduino PCB extending its capabilities. The different shields follow the same philosophy as the original toolkit: they are easy to mount, and cheap to produce. LEaD’s shield will be used to interface the MRF chip into the LED Driver Circuit, thus since the design/decision of the LEaD's shield has not been finalized yet, the cost per unit is still unknown. factors such as cost of materials, size, and reproducibility will be taken into account.
