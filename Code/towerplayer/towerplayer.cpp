...
#define TEST 0
   ...
   int i,j=0;
    ...
    xbee_stat = ftdi_write_data_set_chunksize(&ftdic, 64); // set chunk (buffer) size
              ...
              if(p->startus <= elaptime)  // time to send another frame to xbee
                {
                 ...
                 nbytes = ftdi_write_data(&ftdic, buf, bufcnt);
                 if(nbytes < 0)
                   {
...
//    }
    ...
    if((ret = ftdi_set_baudrate(ftdic, 57600)) < 0)
      {
       fprintf(stderr, "unable to set baudrate on usb (xbee): %d (%s)\n", ret, ftdi_get_error_string(ftdic));
       return ret;           
      }
    else
      {
